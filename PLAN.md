...
**Hosting: `~/Documents/Dev/transparent-models/`** holds `scunet-color-real-gan-F16.gguf` and `scunet-color-real-psnr-F16.gguf` (F16, ~36MB each; SHA256s are recorded in `ModelCatalog.cpp`). This folder becomes the Forgejo LFS repo `forge.db-serve.com/dbajan/transparent-models` — one shared repo for all future models (colorization etc.). Until the fork repo is pushed, `transparent` builds against the local checkout via `-DFETCHCONTENT_SOURCE_DIR_VISIONCPP=$HOME/Documents/Dev/vision.cpp` (CMakeLists carries a TODO to point `FetchContent_Declare` at the forge URL pinned to a tag, e.g. `v0.3.1-transparent.1`, and drop the override).
...
- Pipeline order in `main.cpp`: BackgroundRemoval → **Denoise** → Upscale → Bokeh (denoise-then-upscale is canonical; upscaling amplifies noise). Advanced mode gets it via the reorderable step list automatically; Simple mode gets it in the dropdown; the Simple-mode default stays Background Removal; nothing is auto-enabled.
